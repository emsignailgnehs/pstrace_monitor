...
Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
# show an "Open" dialog box and return the path to the selected file
picklefile = askopenfilenames(
    title= 'open .picklez file (can select multiple files)',
...
print('reading data...')
pickleFiles = list(picklefile)

for picklefile in pickleFiles:
    dataSource = ViewerDataSource()
    dataSource.load_picklefiles([picklefile])
    ...
    output_json_name = os.path.join(output_dir, f'peak_analysis-{filename}.json')
    ...
    for ch, data in channel_data.items():
        ...
        channel_data[ch]['peak'] = peakT.transform(X)
    ...
    for ch in channel_data.keys():
        data = channel_data[ch]
        curve_analysis = data['peak']
        for idx, analysis in enumerate(curve_analysis):
            t, deri, smoothed_c = analysis['X']
            analysis['X'] = {
                't': t.tolist(), 'deri': deri.tolist(), 'smoothed_c': smoothed_c.tolist()
            }
            analysis['name'] = data['name'][idx]
            analysis['user_mark'] = bool(data['user_mark'][idx])
            analysis['device'] = data['device'][idx]
        curve_analysis_results.extend(data['peak'])
        # names.extend(data['name'])
        # user_marks.extend([bool(user_mark) for user_mark in data['user_mark']])
        # devices.extend(data['device'])
    with open(output_json_name, 'w') as f:
        json.dump(curve_analysis_results, f, indent=4)
    logger.info(f'Output peak_analyses to json file finished')